...
class Module:
    @T.prim_func
    def main(A: T.Buffer((128, 128), "float32"), B: T.Buffer((128, 128), "float32"), bias: T.Buffer((128,), "float32"), matmul_bias_relu: T.Buffer((128, 128), "float32")):
        T.func_attr({"tirx.noalias": True})
        # with T.sblock("root"):
        matmul = T.sblock_alloc_buffer((128, 128))
        for i_0 in T.parallel(8):
            for j_0 in range(8):
                for k_0, i_1, k_1 in T.grid(16, 16, 8):
                    for j_1 in T.vectorized(16):
                        with T.sblock("matmul"):
                            v_i = T.axis.spatial(128, i_0 * 16 + i_1)
                            v_j = T.axis.spatial(128, j_0 * 16 + j_1)
                            v_k = T.axis.reduce(128, k_0 * 8 + k_1)
                ...
                for ax0, ax1 in T.grid(16, 16):
                    with T.sblock("matmul_bias_relu"):
                        ...
                        v_j = T.axis.spatial(128, j_0 * 16 + ax1)
