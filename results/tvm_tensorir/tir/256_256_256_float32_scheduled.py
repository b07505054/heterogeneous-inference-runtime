...
class Module:
    ...
    def main(A: T.Buffer((256, 256), "float32"), B: T.Buffer((256, 256), "float32"), bias: T.Buffer((256,), "float32"), matmul_bias_relu: T.Buffer((256, 256), "float32")):
        ...
        for i_0 in T.parallel(16):
            for j_0 in range(16):
                for k_0, i_1, k_1 in T.grid(32, 16, 8):
                    for j_1 in T.vectorized(16):
                        with T.sblock("matmul"):
                            v_i = T.axis.spatial(256, i_0 * 16 + i_1)
                            v_j = T.axis.spatial(256, j_0 * 16 + j_1)
                ...
                for ax0, ax1 in T.grid(16, 16):
                    with T.sblock("matmul_bias_relu"):
                        v_i = T.axis.spatial(256, i_0 * 16 + ax0)
                        v_j = T.axis.spatial(256, j_0 * 16 + ax1)
