...
class RuntimeScheduler:
    ...
    def run(self, requests: list[Request]) -> SchedulerResult:
        ...
        while pending:
            ...
            needed = self.memory.needed_blocks(req)
            serving_events.append(
                {
                    "time_ms": round(time_ms, 3),
                    "event": "request_admitted" if self.memory.can_allocate(req) else "request_rejected",
            ...
            if not self.memory.can_allocate(req):
                rejected += 1
                oom += 1
                continue
    ...
    def _choose_decode_batch(self, req: Request, pending: list[Request], current_time_ms: float) -> list[Request]:
        ...
        for item in pending:
            ...
            if len(candidates) >= self.max_decode_batch_size:
                remaining.append(item)
                continue
            needed = self.memory.needed_blocks(item)
            similar = abs(item.prompt_tokens - req.prompt_tokens) <= 512
            if inside_lookahead and similar and needed <= budget_blocks and self.memory.can_allocate(item):
                try:
