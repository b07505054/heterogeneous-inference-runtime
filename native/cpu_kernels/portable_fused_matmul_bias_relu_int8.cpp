...
namespace {
constexpr const char* kKernelId = "portable_fused_matmul_bias_relu_int8_symmetric";
...
void run_int8_matmul_bias_relu(
    const int8_t* a, const int8_t* b, const float* bias, float* out,
    ...
    for (long i = 0; i < M; ++i) {
        for (long j = 0; j < N; ++j) {
            ...
            for (long kk = 0; kk < K; ++kk) {
                const int32_t av = static_cast<int32_t>(a[i * K + kk]) - activation_zero_point;
                const int32_t bv = static_cast<int32_t>(b[kk * N + j]) - weight_zero_point;
                acc += av * bv; // real int8 x int8 -> int32 arithmetic.
            }
...
Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (flag == "--b-int8") args.b_path = next("--b-int8");
    ...
    if (args.a_path.empty() || args.b_path.empty() || args.bias_path.empty() || args.out_path.empty()) fail("--a-int8, --b-int8, --bias, and --out are required");
    if (args.activation_scale <= 0.0 || args.weight_scale <= 0.0) fail("scales must be positive");
    if (args.activation_zero_point != 0 || args.weight_zero_point != 0) fail("Slice 3A supports zero_point=0 only");
...
int main(int argc, char** argv) {
    ...
    const std::vector<int8_t> b = read_exact<int8_t>(args.b_path, static_cast<size_t>(args.k) * args.n, "i8");
    ...
    for (int r = 0; r < args.repeats; ++r) {
        ...
        run_int8_matmul_bias_relu(a.data(), b.data(), bias.data(), out.data(), args.m, args.n, args.k,
    ...
    json << "  \"arithmetic\": \"int8_times_int8_accumulate_int32_dequantize_fp32\",\n";
